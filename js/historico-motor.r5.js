// js/historico-motor.r5.js — robust loader for Engine history
(async function(){
  ...
  if(!ok){
    const local = H.readLocal(['miec_history_motor_v1','miec_sync_outbox_v1']);
    const rows = [];
    for (const it of local){
      if (it && (it.table==='history_motor' || it.brand)){
