window.MIEC_HIST=(function(){
  function fmtTs(ts){ try{ const d=new Date(Number(ts)); if(!isFinite(d)) throw 0; return d.toLocaleString('pt-PT'); }catch{ return String(ts); } }
  ...
  function ensureTable(hostId, headers){
    ...
    let tbody=document.getElementById('rows'); if(tbody) return tbody;
    ...
    card.appendChild(table); host?.appendChild(card); return tbody;
  }
  async function getClient(){ if(window.supabase&&window.SUPABASE_URL&&window.SUPABASE_ANON_KEY){ try{return window.supabase.createClient(window.SUPABASE_URL,window.SUPABASE_ANON_KEY)}catch{} } return null; }
  async function getSessionUserId(sb){ try{ const {data:{session}}=await sb.auth.getSession(); return session?.user?.id||null }catch{ return null } }
  function readLocal(keys){ for(const k of keys){ try{ const v=JSON.parse(localStorage.getItem(k)||'[]'); if(Array.isArray(v)&&v.length) return v; }catch{} } return []; }
