// historico-win.js — v1.3 (one-time migration + robust dedupe + newest-first)
(function(){
...
function tsNum(x){
  ...
  const MIG_FLAG  = 'miec_migrated_win_v1';
  const IDX_KEY   = 'miec_hist_idx_win';  // guarda ts por fingerprint
  ...
  function normEntry(e){
    ...
    const win = e.win || e.hin || '';
    const ts = e.ts || e.timestamp || e.date || null;
    return { ts, win, result: resStr, reason, certificate, issuer, photo };
  }
  function fpWin(e){
    return `${(e.win||'').toUpperCase()}|${(e.result||'').toLowerCase()}|${(e.reason||'').trim()}|${(e.certificate||'').trim()}|${(e.issuer||'').trim()}|${(e.photo||'').length}`;
  }
  ...
  function migrateOnce(){
    ...
    if(migrated){
      // estabiliza canon sem duplicar, mantendo ts existente ou do índice
      const out = [];
      const seen = new Set();
      for(const row of canon){
        ...
        if(!n || !n.win) continue;
        const fp = fpWin(n);
        // preferir ts existente; se faltar, usar o do índice; só em último caso manter 0
        let t = tsNum(n);
    ...
    // Primeira migração: ler fontes várias e consolidar
    const candidates = [];
    const keys = ['miec_history_win','winHistory','historicoWIN','history_win','hist_win'];
    for(const k of keys){
      const arr = parseMaybe(localStorage.getItem(k));
      if(Array.isArray(arr)){
        for(const raw of arr){ const n = normEntry(raw); if(n && n.win) candidates.push(n); }
      }
    }
    // Dedupe por fingerprint; atribuir ts estável: se tiver ts usa; senão, cria uma vez e guarda em idx
    const out = [];
    const seen = new Set();
    for(const n of candidates){
      ...
      if(!t){
        t = idx[fp] || Date.now();
        idx[fp] = t; // guardar para não mudar em refresh futuros
      }
      n.ts = t;
      out.push(n);
    }
