...
(function(){
  ...
  function cleanUrl(){
    try{ var base = location.pathname + (/\?/.test(location.search) ? location.search.replace(/([?#].*)/,'') : "?v=418"); history.replaceState({}, "", base);}catch(_){}
  }
  ...
  async function gate(){
    try{
      ...
      if (!sess){ overlay('gate: redirect → login'); location.replace("login.html?v=418"); return; }
